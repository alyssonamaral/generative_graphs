...
def nx_is_star(G: nx.Graph) -> bool:
    """
    Um grafo é estrela se existe um único vértice de grau n-1 e todos os demais de grau 1.
    """
    n = G.number_of_nodes()
    if n < 2: return False
    degs = [d for _, d in G.degree()]
    center = sum(1 for d in degs if d == n-1)
    leaves = sum(1 for d in degs if d == 1)
    return center == 1 and leaves == n-1

class StarModelEvaluation(object):
    ...
    def rollout_and_examine(self, model, num_samples):
        ...
        for i in range(num_samples):
            ...
            adj_lists_to_plot.append({n: list(nx_g.neighbors(n)) for n in nx_g.nodes()})

            graph_size = nx_g.number_of_nodes()
            valid_size = self.v_min <= graph_size <= self.v_max
            is_star = nx_is_star(nx_g)

            num_total_size += graph_size
            if valid_size: num_valid_size += 1
            if is_star: num_star += 1
            if valid_size and is_star: num_valid += 1
        ...
        self.star_ratio = num_star / num_samples
